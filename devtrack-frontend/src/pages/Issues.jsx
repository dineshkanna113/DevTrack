...
export default function Issues() {
  ...
  useEffect(() => {
    axios.get("https://devtrack-backend-tkdg.onrender.com/issues", {
  ...
  const handleSubmit = async e => {
    ...
    try {
      await axios.post("https://devtrack-backend-tkdg.onrender.com/issues", form, {
        headers: { Authorization: `Bearer ${token}` }
      });
        ...
        .map(issue => (
          <div key={issue.id} style={{ border: "1px solid #ccc", padding: "10px", marginTop: "10px" }}>
            ...
            <button
              ...
              onClick={async () => {
                await axios.patch(`https://devtrack-backend-tkdg.onrender.com/${issue.id}/close`, {}, {
                  headers: { Authorization: `Bearer ${token}` }
                });
            ...
            <button
              ...
              onClick={async () => {
                if (window.confirm("Delete this issue?")) {
                  await axios.delete(`https://devtrack-backend-tkdg.onrender.com/issues/${issue.id}`, {
                    headers: { Authorization: `Bearer ${token}` }
                  });
                  alert("Issue deleted");
                  window.location.reload();
